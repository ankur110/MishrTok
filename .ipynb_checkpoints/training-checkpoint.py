...
class HinglishBPE:
    ...
    def train(self,
        ...
        for i in tqdm(range(num_merges),desc="Merges",unit="merge"):
            ...
            if checkpoint_prefix and (i + 1) % checkpoint_interval == 0:
                # flush current learned merges/vocab to disk
    ...
    def save(self, prefix: str):
        ...
        with open(model_file, "w", encoding="utf-8") as f:
            ...
            for s, idx in self.special_tokens.items():
                f.write(f"{s} {idx}\n")
            # write merges in creation order (one per line "a b")
    ...
    # Utilities
    # ----------------------------
    def register_special_tokens(self, mapping: Dict[str, int]):
        self.special_tokens.update(mapping)
        self.inverse_special.update({v: k for k, v in mapping.items()})
